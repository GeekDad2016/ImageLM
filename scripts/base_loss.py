...
model, meta = load_model("base", device, phase="eval", model_tag=model_tag, step=model_step)
...
for split_name in ["train", "val"]:
    loader = get_distributed_data_loader(device_batch_size, sequence_len, split_name, device=device)
    with autocast_ctx:
        bpb = evaluate_bpb(model, loader, steps)
...
if ddp_rank == 0:
    ...
    for prompt in prompts:
        with autocast_ctx:
            sample, _ = engine.generate_batch(prompt, num_samples=1, max_tokens=16, temperature=0)
